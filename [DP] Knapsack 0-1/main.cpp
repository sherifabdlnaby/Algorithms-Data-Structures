...
int main() {
    ...
    cout << "Enter N (number of items), W (total weight) \n followed by N pairs for items (v, w).";

    int N, W;
    std::vector<item> items;
    cin >> N >> W;

    //not the most efficient (I hate redundant coping :'D)
    for (int i = 0; i < N; ++i) {
        item inputItem;
        cin >> inputItem.value >> inputItem.weight;
        items.push_back(inputItem);
    }

    vector<item> selectedItems = knapsackItems(items, W);

    int totalWeight = 0, totalValue = 0;
    for (int i = 0; i < selectedItems.size(); ++i) {
        totalValue += selectedItems[i].value;
        totalWeight += selectedItems[i].weight;
        cout << "Item " << selectedItems[i].id << ", value: " << selectedItems[i].value << ", weight: " << selectedItems[i].weight << endl;
    }
